...
func (dockerProvider *ProviderDocker) updateStateMetric(result *ScrapeResultWrapper, ctr types.Container) {
	metric_id := fmt.Sprintf("container_state_%v", ctr.ID)
	dockerProvider.containerState[ctr.ID] = ctr.State
	if ctr.State != "running" {
		result.PushFailure(metric_id, "%v isn't running (%v)", containerDescription(ctr), ctr.State)
	} else {
		result.PushOK(metric_id)
	}
}

func (dockerProvider *ProviderDocker) updateImageMetric(result *ScrapeResultWrapper, storage storage.Storager, ctr types.Container) {
	...
	if changed && exists {
		result.PushMessage(metric_id, "%v was updated from %v to %v", containerDescription(ctr), lastKnownImage, ctr.ImageID)
	}
}

func (dockerProvider *ProviderDocker) updateRestartCountMetric(result *ScrapeResultWrapper, ctr types.Container, inspect types.ContainerJSON) {
	...
	if lastRestartCount != inspect.RestartCount && inspect.RestartCount > 0 {
		result.PushFailure(metric_id, "%v is restarting (%v, %v)", containerDescription(ctr), inspect.RestartCount, ctr.Status)
	} else {
		result.PushOK(metric_id)
	}
}

func (dockerProvider *ProviderDocker) Update(result *ScrapeResultWrapper, storage storage.Storager) {
